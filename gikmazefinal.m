...
% 3. Preprocess and generate safe mask
gray = rgb2gray(img_proc);
bw = imbinarize(gray, 'adaptive');
bw = ~bw;
distMap = bwdist(~bw);
scale_factor = 14 / size(img_orig, 1);  % cm per pixel
avoid_dist_px = round(0 / scale_factor);

% Edge mask to avoid borders
edgeMask = true(size(bw));
edgeMask(1:avoid_dist_px, :) = false;
edgeMask(end-avoid_dist_px+1:end, :) = false;
edgeMask(:, 1:avoid_dist_px) = false;
edgeMask(:, end-avoid_dist_px+1:end) = false;

% Final safe mask: avoid black and image edges
safeMask = (distMap > avoid_dist_px) & edgeMask;

% 4. Click start and goal
figure, imshow(img_orig); title('Click START (red) and GOAL (green)');
[x, y] = ginput(2);
start = round([x(1), y(1)]);
goal = round([x(2), y(2)]);
close;
...
if snapped1, disp('⚠ Start snapped'); end
if snapped2, disp('⚠ Goal snapped'); end
...
path_30 = interparc(20, double(path(:,1)), double(path(:,2)), 'linear');
...
% 8. GIK solve
robot = importrobot('myCobot28.urdf', 'MeshPath', "C:\\Users\\nitin\\Downloads\\New folder (7)\\meshes", 'DataFormat', 'row');
...
for i = 1:size(qSolutions_deg, 1)
    ...
    msg = sprintf('set_angles(%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,400)', angles);
